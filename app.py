...
spec = FlaskPydanticSpec('flask', title='Teste de API do Christopher no Swagger usando PYTHON')
...
@server.get('/pessoas')  # Endpoint.
@spec.validate(
    query=QueryPessoa,
    resp=Response(HTTP_200=Pessoas)
)
def buscar_pessoas():
    """Dados que estão salvo no banco."""
